...
export function ShopOAuthModal({
  ...
  return (
    ...
    >
      <form
        className="w-[440px] max-w-[90vw] rounded-lg border border-border bg-surface p-6 shadow-xl"
        onClick={(e) => e.stopPropagation()}
        onSubmit={handleSubmit}
      >
        <h3 className="text-base font-semibold text-fg">Connect your Shopify store</h3>
        <p className="mt-1 text-sm text-fg-muted">
          Enter your shop subdomain. You'll be redirected to Shopify to approve access.
        </p>

        <label className="mt-4 block text-xs font-medium text-fg-muted">Shop</label>
        <div className="mt-1 flex items-center rounded-md border border-border bg-surface-subtle">
          <input
            type="text"
            value={value}
            onChange={(e) => setValue(e.target.value)}
            placeholder="munim-dev"
            className="flex-1 bg-transparent px-3 py-2 text-sm text-fg outline-none"
            autoFocus
          />
          <span className="px-3 py-2 text-xs text-fg-muted">.myshopify.com</span>
        </div>

        {error && <p className="mt-3 text-xs text-destructive">{error.message}</p>}
