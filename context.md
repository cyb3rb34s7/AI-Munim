...
Phase 1 complete. Repo has working monorepo + backend foundations + frontend scaffold + a `/health` tracer endpoint that exercises every load-bearing piece (envelope, trace_id, JSON logs, error handlers, theme, API client, dumb component pattern). All checks green:
- Backend: `uv run ruff check`, `uv run ruff format --check`, `uv run mypy`, `uv run pytest` — all pass (6/6 tests).
- Frontend: `pnpm typecheck`, `pnpm lint`, `pnpm build` — all pass.
- Live: `GET /health` returns the success envelope with matching `X-Trace-Id` header; `GET /does-not-exist` returns the error envelope with `code: "http.404"`.

**Next:** Phase 2 begins with the universal schema (`merchant`, `connector_credentials`, `record`, `run_log` per `docs/architecture.md §4.1`) + the `BaseConnector` interface + the first connector (Shopify) writing rows into `record`. Plan / iterate / dispatch / review per the module workflow.
...
1. **Phase 2 — Universal schema + first connector.** Wire the 4-table schema (`merchant`, `connector_credentials`, `record`, `run_log`) via SQLModel. Implement `BaseConnector` ABC + `SyncContext` + `RowSink` writer. Write `ShopifyConnector.sync_full` against recorded fixtures (VCR-style cassettes) that produces `record` rows with `entity_type='order'` and full provenance.
2. **Phase 3 — Connectors 2 and 3.** Meta Ads + Shiprocket, same pattern, fewer surprises.
