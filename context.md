...
Phase 8 complete (connectors + review cycle + cross-connector smoke). Three connectors behind one `BaseConnector` ABC live: Shopify (real OAuth) + Meta Ads (demo, 40-row fixture) + Shiprocket (demo, 50-row curated fixture). RTO agent rewired to read Shiprocket shipments; Shopify mapper hashes customer email||phone with the shared `shared/utils/customer_hash` algorithm so cross-connector joins land. Reviewer surfaced 4 CRITICAL + 9 IMPORTANT findings — all CRITICAL + actionable IMPORTANT addressed. Live smoke verifies the brief's narrative end-to-end: Customer A's COD order (3/5 RTO history via Shiprocket) → `convert_to_prepaid` at score 0.772 with ₹3242.40 saved; Customer B (clean) → `confirmation_call` only; legacy seed customer (no Shiprocket history) → population baseline. 213 backend tests pass (was 168 pre-Phase 8); `pnpm typecheck && pnpm lint && pnpm build` all green.
