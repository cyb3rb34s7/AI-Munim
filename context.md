...
Task 2 (onboarding wizard): `/auth/start` no longer seeds; new `/auth/onboard` endpoint returns `{shopify_rows, meta_ads_rows, shiprocket_rows}` after seeding. Frontend `/start` → `/onboarding` → `/chat`. `OnboardingPage` calls the onboard mutation, animates each connector card sequentially (700ms apart, ~2.1s total) for UX, then flips to "Go to chat". Idempotent: a user landing on /onboarding with an already-seeded workspace sees the success state immediately (detected by `connectors.every(c => status === 'demo' && last_sync_at !== null)`). The `auth_client` conftest fixture now calls `/auth/start` + `/auth/onboard` so existing protected-endpoint tests still see the 96 seeded rows. 244 backend tests, all gates green.
...
- 235 backend tests pass (was 213 pre-Phase 9). New tests: 3 dependency, 8 auth router, 5 seed, 7 isolation/auth migration coverage on records/connectors/agent-runs/chat.
- The auth_client conftest fixture is the way to write protected-endpoint tests: it mints a fresh merchant via `/api/auth/start`, returns the TestClient with the signed cookie + the merchant_id/user_id, and tests use `auth_client.client.get(...)`.
- Shopify fixture for Phase 9 (apps/api/src/munim/connectors/shopify/fixtures/orders.json) is REAL-SHAPE Admin API order JSON. Customer emails (rohan@/priya@/amit@example.com) match Shiprocket's curated customers so the cross-connector hash join fires from the first agent run on a fresh visitor's seed data.
- Phase 7 IPv4 lesson preserved: vite proxy `target: '127.0.0.1:8000'` unchanged; only the `rewrite` is removed (Phase 9 stops stripping `/api`).
- `SHOPIFY_OAUTH_ENABLED` is the runtime feature flag: env var + `VITE_SHOPIFY_OAUTH_ENABLED` for the SPA. Deployed env sets both to false; the SPA hides the "Connect to your store" button.
...
- 2026-05-15 — **Phase 8 complete (connectors + review cycle).** Three connectors behind one ABC: Shopify (real OAuth) + Meta Ads (demo, 40-row insights fixture) + Shiprocket (demo, 50-row curated shipments fixture). RTO agent's `customer_rto_rate` rewired to read Shiprocket shipments; Shopify mapper hashes `customer.email||phone` so the cross-connector join lands. Reviewer surfaced 4 CRITICAL + 9 IMPORTANT findings (customer-hash package coupling, missing `MissingShipmentFieldError` test, saturation cap unprotected, magic-string branching on frontend, dead error code regression risk, etc.); all CRITICAL + actionable IMPORTANT addressed in one fix commit. Cross-connector smoke verified: Customer A's high-RTO history flows through Shiprocket → agent picks `convert_to_prepaid` ₹3242.40 saved. 213 backend tests, all gates green.
- 2026-05-16 — **Phase 9 complete (multi-tenant backbone + Docker image).** Anonymous session cookie + per-visitor merchant + per-merchant pre-seeding via `POST /auth/start`. `get_current_merchant_id` FastAPI dependency on every protected endpoint — no silent fallback. `DEFAULT_MERCHANT_ID` retained as test-only constant; `init_db` no longer auto-seeds it. All routers mount under `/api`; Vite proxy stops stripping the prefix. Multi-stage Dockerfile (Node 22 → Python 3.11 slim) + render.yaml committed; local docker build + run smoke confirmed. 235 backend tests pass; `pnpm typecheck && pnpm lint && pnpm build` green. Live Render deployment is the user's step.

---

## Next

1. **Phase 9 review + manual smoke + live deploy** — reviewer subagent against `docs/conventions.md`; user manual browser smoke; user connects the repo to Render via the dashboard and verifies the deployed URL walk (landing → /start → /chat → /agents → /connectors → /records → incognito second visitor → logout). After live deploy, the README's "Try the live demo" link gets the real URL.
2. **Phase 10 (deferred unless time) — real auth.** Magic-link or email/password; per-merchant credential encryption; per-reviewer login URLs.
...
**Root cause:** PydanticAI executes sync `@agent.tool` functions in worker threads via `anyio.to_thread.run_sync` (see traceback frame `pydantic_ai/_function_schema.py:87 → run_in_executor`). When the LLM emits multiple tool calls in one response (parallel tool-use), PydanticAI fans them out concurrently. Each tool calls `ctx.session.exec(...)` on the SAME SQLAlchemy `Session` instance. `Session` is not thread-safe — concurrent `.execute()` calls race on the connection-provisioning state and SQLAlchemy raises this exception by design. Latent bug from Phase 5 (when `chat/tools.py` was first written); the chat agent's prompt forces sequential `1. Pull A → 2. For each, pull B` so it never triggered. The briefing prompt invites parallel calls (`Call query_orders, query_shipments, query_ad_spend`) and the LLM took the invitation.

**Why local SQLite didn't catch it:** Local SQLite + `check_same_thread=False` masks the violation for read-only queries (SQLite serializes implicitly inside the driver). The repro that worked locally also happened to schedule tools sequentially under that LLM call. Postgres + production-fast LLM exposed it.

**Solution:** Added `session_lock: threading.Lock = field(default_factory=threading.Lock)` to `ChatContext`. Every tool's DB-touching section now runs inside `with ctx.session_lock:`. One lock per request, serializes concurrent worker-thread access, no deadlock risk (lock is non-reentrant, no nested acquisition, no cross-tool calls). Both the chat agent and the briefing agent share this fix because they share the tools. Tests unaffected: existing tool tests call tools sequentially and never enter the lock-contended path.

**Guardrail:** Any time a PydanticAI agent shares a sync resource across `@agent.tool` callbacks, that resource MUST be either (a) thread-safe by construction, or (b) serialised with a lock owned by the per-request context. The seed of this bug was at the tool-context-construction site, not at any tool's body — review that constructor when adding new tool surfaces. If a tool needs to be async-native instead, define it `async def` and PydanticAI runs it in the event loop, no worker thread, no lock needed — but then no sync SQLAlchemy session work inside either.
...
**Problem:** Mass-prefixing every test path with `/api` via a PowerShell script — `Get-Content $f -Raw | -replace ... | Set-Content -Encoding UTF8` — corrupted `§` into `Â§` and `→` into `â†'`, plus stamped a UTF-8 BOM onto every file. Caught by a `Grep "Â§"` sanity pass; 9 files were affected before commit.

**Root cause:** Windows PowerShell 5.1's default file-read encoding is the system code page (cp1252 in most locales). `Get-Content -Raw` reads the original UTF-8 bytes as cp1252, producing mojibake when those bytes contain anything above ASCII. `Set-Content -Encoding UTF8` then writes the mojibake out as UTF-8 with BOM, double-mangling.

**Solution:** Reverted via `git checkout` (the original UTF-8 files were committed to history). Re-did the prefix sweep via `bash sed -i` with the explicit pattern set, which preserves byte-perfect encoding. POSIX `sed` doesn't reinterpret encoding; bytes go in, bytes come out.

**Guardrail:** Never use `Set-Content -Encoding UTF8` for files that may contain non-ASCII (Python docstrings with `§`, comments with `→`, anything UTF-8). For mass edits on Windows, prefer `bash sed -i` via the WSL-or-Bash tool. If PowerShell is unavoidable, use `[System.IO.File]::WriteAllText($path, $content, [System.Text.UTF8Encoding]::new($false))` (UTF-8 without BOM) and ensure the read side is `[System.IO.File]::ReadAllText($path, [System.Text.UTF8Encoding]::new($false))` — that bypasses the cp1252 default. The fastest sanity check after any mass edit: `rg "Â§|â†'|â€"` (the three most common cp1252-misinterprets-as-UTF8 byte sequences).

### 2026-05-16 (Phase 9) — adding `/api` prefix to backend routes broke an OAuth redirect URL string; sed prefixed a frontend path it shouldn't have

**Problem:** When I sed-prefixed `/connectors/...` → `/api/connectors/...` across all test files, the OAuth callback redirect URL assertion at `test_oauth_callback_full_flow_redirects_to_frontend` got changed too — even though that string refers to the FRONTEND URL the backend redirects TO, not a backend route. The assertion `location.endswith("/api/connectors?connected=shopify")` then failed because the actual redirect goes to `http://localhost:5173/connectors?connected=shopify`.

**Root cause:** sed pattern matching is structural, not semantic. The Vite-served `/connectors` path on the frontend has the same prefix as the backend's old `/connectors` API path, but only the latter moved under `/api` in Phase 9. The frontend's React Router still serves `/connectors` (a SPA route, not an API route).

**Solution:** One-line edit restoring the assertion to `/connectors?connected=shopify`. Test passes.

**Guardrail:** When mass-prefixing paths after a backend route-mount change, exclude strings that are frontend URLs. The pattern: anything inside `frontend_base_url` or anything matching the SPA's router config is NOT an API path. Catch this in the test fail-mode, not by inspection — but be alert to "this assertion failed and the string is a URL that ends with a slash-prefixed path" as a smell for cross-domain confusion.
...
### 2026-05-16 — Phase 9: `/api` prefix lives in router declarations, NOT in conditional production-only middleware

**Decision:** All FastAPI routers mount under `/api` via `app.include_router(..., prefix=API_PREFIX)`. The Vite dev proxy stops stripping `/api` (was `rewrite: incoming.replace(/^\/api/, '')`). Result: routes match in dev and prod with no conditional logic. The dev proxy still exists (`/api/*` → `127.0.0.1:8000/api/*`) so the frontend doesn't need to know whether it's behind Vite or behind FastAPI's StaticFiles mount.

**Why:** Phase 4 paid the lesson that `/api` is a frontend convention, not a backend route — but at that time, "the backend has never seen /api" was the simpler invariant. Phase 9 mounts the SPA inside FastAPI for production (one URL, one port, one process), so the frontend's `/api/*` paths now need to land on the backend at `/api/*`. The two clean options were (A) prefix the routes in code, or (B) mount the FastAPI app under `/api` via a sub-application. (A) is one diff across `main.py` + the Vite proxy config + every test path; (B) requires careful StaticFiles + sub-app ordering and a runtime branch on `APP_ENV`. (A) is closer to "the same code path in dev and prod" — that's the bar.

**Tradeoff acknowledged:** Every test path is now `/api/...` (the previous Phase 4 lesson said "tests use bare paths because the backend never sees /api"). Phase 9 had to migrate all 9 test files via `sed -i`. The Phase 4 lesson is now out of date but useful: it documents the WHY of the previous shape.

**Revisit if:** we ever need versioned APIs (`/api/v2/...`). At that point, the `/api` prefix becomes `/api/v1` and the dev/prod parity argument still holds.

### 2026-05-16 — Phase 9: anonymous session cookie, NOT real auth, for the v0 demo

**Decision:** Phase 9 ships a signed-cookie anonymous session (starlette `SessionMiddleware`, `itsdangerous` HMAC, 30-day max-age). There is no email, no password, no magic link, no OAuth identity. The cookie carries `{merchant_id, user_id}` and that's the whole identity layer.

**Why:** The brief asks for a working demo that a reviewer can click through. Real auth (email + password + reset + invites) is 2–3 days of engineering for zero brief value — the reviewer will not type a password. The anonymous-session shape has the right scale-out property (per-visitor merchant + isolation tests + zero shared state) without the auth UX. The deferred work (Phase 10 if time permits) is real auth.

**Tradeoff acknowledged:** A determined attacker with another visitor's cookie value gets their session. The cookie is HMAC-signed (so it can't be forged), but if you steal the value (XSS, malware, shared device), you're in. For a hiring demo this is fine; for paying customers it's not. The README's "Honest limitations" section calls this out explicitly.

**Revisit if:** the deployed URL gets used by anyone whose data we care about. The session-cookie shape extends cleanly to real auth — same dependency, real `User` lookup, the rest of the multi-tenant code doesn't change.

### 2026-05-15 — Phase 8: Shopify mapper's `customer_source_id` migrated from raw Shopify ID to SHA-256(email||phone)

**Decision:** The Phase 2 Shopify mapper stored the raw Shopify customer ID (e.g. `"7700000000001"`) in `Order.customer_source_id`. Phase 8 changes that to a SHA-256-truncated-to-16 hash of the customer's email (preferred) or phone (fallback) — the same algorithm the Shiprocket mapper uses. The function is shared (`compute_customer_source_id` in `shiprocket/mapper.py`, imported by the Shopify mapper).

**Why:** The RTO agent's rewired `customer_rto_rate` reads shipments and joins back to the customer of the order being scored via `customer_source_id`. Shopify and Shiprocket are two independent provider systems with two independent customer ID spaces, so the only stable cross-system identity is the contact info (email/phone). Hashing keeps the join key out of PII territory in the DB; truncating to 16 hex chars is collision-safe enough for a single-merchant v0.

**Why the import is from `shiprocket/mapper.py` instead of a shared location:** §3.3 of `docs/conventions.md` says extract on the third use; Shopify's the second site. When a fourth caller arrives we extract to `shared/utils/customer_hash.py`. Today it's one function imported across two files — tractable.

**Tradeoff acknowledged:** Theoretical hash collisions (two customers with identical email AND phone). Accepted for v0; flagged in the Phase 8 plan's self-review. Real-merchant data would force us to add a salt, but the demo data is curated and the hash space (16 hex chars = 64 bits) is much wider than any single merchant's customer count.

### 2026-05-15 — Phase 8: `customer_rto_rate` queries Shiprocket shipments, not Shopify orders

**Decision:** The Phase 6 signal `customer_rto_rate` queried `record` where `source_system=shopify AND entity_type=order` and counted `fulfillment_status=rto`. Phase 8 rewires it to `source_system=shiprocket AND entity_type=shipment`. Order rows in Shopify never carry `fulfillment_status` — that's a shipment lifecycle attribute, populated by Shopify only when Shopify itself fulfills the shipment, which never happens for D2C brands using third-party logistics (every Indian D2C using Shiprocket falls into this bucket). The Phase 6 implementation therefore always returned the population baseline 0.2, regardless of the customer's actual RTO history. The signal worked in tests because the tests seeded `fulfillment_status` directly on the order rows.

**Why:** The fix is the signal doing what it claims to do. Shiprocket is the system of record for shipment lifecycle status — RTO is a real terminal state there, not a Shopify concept. With the rewire + the Shiprocket connector + the customer hash, the agent's customer-history signal finally fires on the demo: customer A (3/5 RTO) saturates the signal at 0.9, customer B (0/5 RTO) gets 0.0, and the population baseline kicks in only for genuinely new customers with <3 history.

**Tradeoff acknowledged:** Customers without a Shiprocket history (i.e., the merchant hasn't yet shipped to them) get the population baseline. This is correct behaviour — we don't have evidence either way. The agent's diagnostic surfaces `rate_source: population_baseline` so the merchant can see this in the run log.
...
- 2026-05-16 — Phase 9 implementation (anonymous session cookie + auth router + per-merchant seeding + multi-tenant refactor across all routers + AuthProvider/LandingPage/StartPage on the frontend + multi-stage Dockerfile + render.yaml + /api prefix sweep): executed by Claude Opus 4.7 as a single coder subagent dispatched per `superpowers:subagent-driven-development`, eight tasks top-to-bottom, commit-per-task. Key adjustments vs. plan: (1) PowerShell mass-edit corrupted UTF-8 (cp1252 misinterpret + BOM stamp) on 9 files; reverted via git and re-did via bash `sed -i`. New paid lesson in Problems & solutions. (2) Initial /auth/logout was 204 no-body; the ky-based client expects an envelope, so changed to 200 + `{logged_out: true}`. (3) The legacy /connectors/shopify/connect path is still routed (used by one test) — added a per-merchant credential delete to that test instead of mounting a separate fixture. (4) `useAuthContext` lives next to AuthProvider with an explicit eslint-disable for `react-refresh/only-export-components` — splitting it into a sibling file felt like over-engineering. (5) Live Render deployment is the user's step — I built the Docker image locally (1.3GB) and smoke-tested `/api/health` + `/api/auth/start` end-to-end inside the container before stopping.
- 2026-05-15 — Phase 8 implementation (Meta Ads + Shiprocket demo connectors + customer_rto_rate rewire + demo-connect endpoint + frontend EnableDemoButton): executed by Claude Opus 4.7 as a single coder subagent dispatched per `superpowers:subagent-driven-development`, six tasks top-to-bottom, commit-per-task. Key adjustments vs. plan: (1) the plan's signal-test assertion `assert score == 1.0  # min(0.6 * 1.5, 1.0)` was off — 0.6 × 1.5 = 0.9, no saturation, so the test asserts `pytest.approx(0.9)` instead. (2) Plan code blocks routed the demo endpoint through a closure-style `connect_demo` rename inside `service.py`; implementer used a sibling `demo_connect.py` module + a public `build_connector_view` helper to avoid the in-function `from ... import` smell. (3) Shopify mapper test that previously asserted the raw Shopify customer ID was updated to assert the SHA-256 hash matches what Shiprocket's mapper produces for the same email — this is the load-bearing contract test for the cross-connector join, not just a shape check. (4) Legacy `useConnectMutation.ts` hook + `postConnect()` API are now dead code on the frontend (no caller after the demo split); left in place to keep the diff scoped to Phase 8 — Phase 9 cleanup target. (5) Existing `connect_demo` service helper now also accepts a `registry` param so `_build_view` can read `is_demo` from the connector class; threaded through all four call sites.
