...
Phase 8 implementation complete; awaiting reviewer subagent + manual smoke. Meta Ads + Shiprocket added as demo-mode connectors behind the same `BaseConnector` ABC as Shopify, bringing the project to the brief's three-connector requirement. Curated Shiprocket fixture makes the RTO agent's customer-history signal fire meaningfully (customer A 3/5 RTO → `convert_to_prepaid`, customer B 0/5 RTO → no_action). New generic `POST /connectors/{name}/connect-demo` endpoint replaces the legacy Shopify-only `connect` flow for Meta + Shiprocket; Shopify keeps the real OAuth path. Rewired `customer_rto_rate` to read shipments (`source_system=shiprocket, entity_type=shipment`) instead of orders — orders never carry `fulfillment_status` (it's a shipment lifecycle attribute), so the Phase 6 signal silently always returned the population baseline. Shopify mapper now hashes `customer.email||phone` with the same algorithm as the Shiprocket mapper so cross-connector joins land. 206 backend tests pass (was 168); `pnpm typecheck && pnpm lint && pnpm build` all green.
...
- 2026-05-15 — **Phase 8 implementation complete (awaiting review).** Three connectors behind one ABC: Shopify (real OAuth) + Meta Ads (demo, 40-row insights fixture, 4 D2C campaigns × 10 days) + Shiprocket (demo, 50-row shipments fixture, curated for RTO agent narrative). `BaseConnector.is_demo` ClassVar surfaces through the GET /connectors response; `POST /connectors/{name}/connect-demo` is the one-click enable path with typed `connector.not_demo` (400) / `connector.unknown` (404) errors. RTO agent's `customer_rto_rate` rewired to read Shiprocket shipments; Shopify mapper now hashes `customer.email||phone` so the join works. 206 backend tests, `pnpm typecheck && pnpm lint && pnpm build` all green.

---

## Next

1. **Phase 8 review + manual smoke** — reviewer subagent against `docs/conventions.md`; user manual browser smoke (Enable demo → Sync → Records → Run agent → Chat); fix cycle.
2. **Phase 9 — README rewrite + `docker-compose up` story + final polish.** This is the deliverable's headline artifact. The README must call out demo-mode honestly (Meta + Shiprocket fixtures, no real API), document the customer-hash join, and walk reviewers through the brief's evidence (`docs/architecture.md`, `docs/conventions.md`, the agent run log).
